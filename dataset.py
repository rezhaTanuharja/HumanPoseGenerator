...
class NPZDataset(Dataset):
    def __init__(self, data_dict):
        self.data = data_dict
        self.length = len(next(iter(data_dict.values())))  # length of the first field
...
def load_and_combine_npz(root_dir):
    combined_data = {}

    for dirpath, _, filenames in os.walk(root_dir):
        for file in filenames:
            if file.endswith(".npz"):
                npz_path = os.path.join(dirpath, file)
                npz_data = np.load(npz_path)

                if "poses" not in combined_data:
                    combined_data["poses"] = [npz_data["poses"]]
                else:
                    combined_data["poses"].append(npz_data["poses"])
...
# Example usage with DDP:
# dataloader = get_dataloader(dataset, batch_size=32, rank=0, world_size=1)
...
def main(local_rank: int = 0, world_size: int = 1):
    ...
    data_dir = "/home/ratanuh/Datasets/"
    # combined_data = load_and_combine_npz(data_dir)
    axis_angle_poses = torch.tensor(
        load_and_combine_npz(data_dir)["poses"],
