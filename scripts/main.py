...
def main(local_rank: int = 0) -> None:
    ...
    data = np.load(dataset_directory + dataset_name)
    ...
    axis_angle_poses = axis_angle_poses.view(
        axis_angle_poses.shape[0],
        axis_angle_poses.shape[1] // 3,
        3,
    )
    ...
    poses = manifold.exp(
        torch.eye(3, device=device, dtype=parameters["data_type"]),
        axis_angle_poses,
    )
    ...
    model = FiLM(modulator=temporal_components, main_block=spatial_components)
    ...
    for epoch in range(350):
        ...
        for batch in dataloader:
            ...
            encoded_time = time_encoder(
                time,
                period=1.6,
                num_waves=parameters["num_sinusoids"],
            )
